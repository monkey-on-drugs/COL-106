...
void LinearProbing::createAccount(std::string id, int count) {
    ...
    do{
        ...
        hash_index = (hash_index + i) % bankStorage1d.size();
    }
...
int LinearProbing::find(int hashed_index, std::string id){
    int i = 1; int start_index = hashed_index;
    do{
        if(bankStorage1d[hashed_index].id == id){
            return hashed_index;
        }
        hashed_index = (hashed_index + i) % bankStorage1d.size();
    }
    while (hashed_index != start_index);
    return -1;
}

float LinearProbing::power(float x, int n){
...
void LinearProbing::merge(std::vector<int>& vector, int start, int middle, int end){
...
void LinearProbing::merge_sort(std::vector<int>& vector, int left, int right){
    if(left < right){
