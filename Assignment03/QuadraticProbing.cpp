...
void QuadraticProbing::createAccount(std::string id, int count) {
    ...
    do{
        ...
        hash_index = (hash_index + i*i) % bankStorage1d.size(); i++;
...
int QuadraticProbing::find(int hashed_index, std::string id){
    int i = 1; int start_index = hashed_index; int j = 1;
    do{
        if(bankStorage1d[hashed_index].id == id){
            return hashed_index;
        }
        hashed_index = (hashed_index + i*i) % bankStorage1d.size(); i++;
...
float QuadraticProbing::power(float x, int n){
...
void QuadraticProbing::merge(std::vector<int>& vector, int start, int middle, int end){
...
void QuadraticProbing::merge_sort(std::vector<int>& vector, int left, int right){
    if(left < right){
